...
import com.lipan.manager.BaseApplication;
...
public class UIUtils {
    ...
    public static void runOtherMessageOnMainThread() throws Exception {
        ...
        if (mQueueNext == null) {
            mQueueNext = MessageQueue.class.getDeclaredMethod("next");
            mQueueNext.setAccessible(true);
        }

        if (mTarget == null) {
            mTarget = Message.class.getDeclaredField("targer");
            mTarget.setAccessible(true);
        }

        Message msg = (Message) mQueueNext.invoke(mQueue);
        Handler handler = (Handler) mTarget.get(msg);
